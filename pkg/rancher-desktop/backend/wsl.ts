...
export default class WSLBackend extends events.EventEmitter implements VMBackend, VMExecutor {
  constructor(kubeFactory: (backend: WSLBackend) => KubernetesBackend) {
    ...
    this.hostSwitchProcess = new BackgroundProcess('host-switch.exe', {
      spawn: async() => {
        ...
        const k8sPortForwarding = '127.0.0.1:6443=192.168.127.2:6443';
  ...
  protected async runInit() {
    ...
    if (this.cfg?.experimental.rdNetworking === false) {
      ...
      await this.writeFile('/usr/local/bin/wsl-init-rd-networking', WSL_INIT_RD_NETWORKING_SCRIPT, 0o755);
      initScript = 'wsl-init-rd-networking';
    }
    ...
    // clobbered.
    const stream = await Logging[initScript].fdStream;
    ...
    this.process = childProcess.spawn('wsl.exe',
      ['--distribution', INSTANCE_NAME, '--exec', `/usr/local/bin/${ initScript }`],
      {
        env: {
          ...process.env,
